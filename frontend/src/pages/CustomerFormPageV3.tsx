...
export default function CustomerFormPageV3() {
  ...
  return (
    <div className="min-h-screen bg-background-200 bg-dot-pattern bg-dot">
      <Header />

      {/* Hero Section */}
      <section className="relative py-12 md:py-16 bg-white border-b border-brand-100">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          {/* アイコン */}
          <div className="mx-auto mb-6 w-16 h-16 rounded-full bg-brand-500 shadow-brand-lg flex items-center justify-center animate-fade-in">
            <svg className="w-10 h-10 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
            </svg>
          </div>
          <h1 className="text-3xl md:text-4xl font-bold text-brand-700 mb-4 font-mincho tracking-wider animate-slide-up">
            料金シミュレーション
          </h1>
          <div className="w-24 h-0.5 mx-auto bg-brand-400 mb-6"></div>
          <p className="text-base md:text-lg text-neutral-600 leading-relaxed max-w-2xl mx-auto animate-slide-up font-gothic">
            ご希望の撮影メニューをお選びください。
          </p>
        </div>
      </section>
      ...
      {/* Campaign Section */}
      {campaigns.length > 0 && (
        <section className="py-6 bg-secondary-50 border-y border-secondary-200">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-center mb-4">
              ...
              <p className="text-center text-sm font-bold text-secondary-800 font-mincho tracking-wider">
                現在実施中のキャンペーン
              </p>
            </div>
              ...
              {campaigns.map((campaign) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="flex items-center justify-center w-10 h-10 rounded-full bg-secondary-500 shadow-soft">
                      <svg className="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                      </svg>
                    </div>
                    <div>
                      <h3 className="text-sm font-bold text-neutral-800 font-mincho">
                        {campaign.name}
                      </h3>
                      <p className="text-xs text-neutral-500 mt-0.5 font-gothic">
                        {campaign.start_date} 〜 {campaign.end_date}
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className="text-lg font-bold text-secondary-600 font-mincho">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-white rounded-2xl shadow-medium border border-brand-100 p-8 mb-8 animate-fade-in">
            <div className="text-center mb-8">
              <h2 className="text-2xl font-bold text-brand-700 mb-2 font-mincho tracking-wider">
                撮影カテゴリを選択
              </h2>
              <div className="w-16 h-0.5 mx-auto bg-brand-400"></div>
            </div>
              ...
              {shootingCategories.map((category) => (
                ...
                >
                  ...
                  <div className="py-4 px-3 bg-white">
                    <p className={`text-sm font-bold text-center transition-colors font-mincho tracking-wide ${
                      selectedShootingCategoryId === category.id
                        ? 'text-brand-700'
          ...
          <div ref={formContentRef}>
            ...
            <div className="bg-secondary-50 border-2 border-secondary-200 rounded-xl p-8 text-center shadow-soft">
              ...
              <p className="text-secondary-800 font-semibold mb-2 font-mincho">
                この撮影カテゴリのフォームはまだ作成されていません
              </p>
              <p className="text-sm text-secondary-600 font-gothic">
      ...
      {selectedItemIds.length > 0 && (
        <div className="sticky bottom-0 bg-white border-t-2 border-brand-200 shadow-large z-50 animate-slide-up">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
            <div className="bg-brand-50 rounded-xl p-4 mb-4 border border-brand-200">
              <div className="flex justify-between items-center">
                <span className="text-lg font-bold text-brand-800 font-mincho">合計金額</span>
                <div className="text-right">
                  <div className="text-3xl md:text-4xl font-bold text-brand-600 font-mincho">
                    {formatPrice(totalPrice)}
                  </div>
                  <div className="text-xs text-neutral-500 mt-1 font-gothic">（税込）</div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <button
                onClick={handleReset}
                className="px-5 py-3.5 border-2 border-neutral-300 text-neutral-700 font-bold rounded-xl hover:bg-neutral-50 hover:border-neutral-400 transition-all duration-300 shadow-soft hover:shadow-medium transform hover:scale-105 font-gothic"
              >
                やり直す
              </button>
              <button className="px-5 py-3.5 bg-brand-500 text-white font-bold rounded-xl hover:bg-brand-600 transition-all duration-300 shadow-brand hover:shadow-brand-lg transform hover:scale-105 font-gothic">
                ご予約はこちら
              </button>
            </div>

            <p className="text-xs text-center text-neutral-500 mt-4 leading-relaxed font-gothic">
